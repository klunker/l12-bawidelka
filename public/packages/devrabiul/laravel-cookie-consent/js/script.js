...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const cookieConsentPrefix =
        consentRoot?.getAttribute('data-cookie-prefix') || 'cookie_consent';
    ...
    acceptButtons.forEach((button) => {
        button.addEventListener('click', () => {
            setCookie(
                cookieConsentPrefix,
                'accepted',
                consentRoot?.getAttribute('data-cookie-lifetime') || 7,
            );
    ...
    rejectButtons.forEach((button) => {
        button.addEventListener('click', () => {
            setCookie(
                cookieConsentPrefix,
                'rejected',
                consentRoot?.getAttribute('data-reject-lifetime') || 1,
            );
    ...
    function showModal() {
        ...
        document
            .querySelectorAll('.cookie-toggle input:not([disabled])')
            .forEach((toggle) => {
                const category = toggle.dataset.category;
                toggle.checked = savedPreferences
                    ? savedPreferences[category] !== false
                    : true;
            });
    }
    ...
    function setAllPreferences(accept) {
        const preferences = {};
        document
            .querySelectorAll('.cookie-toggle input[data-category]')
            .forEach((toggle) => {
                preferences[toggle.dataset.category] = toggle.disabled
                    ? true
                    : accept;
            });
    ...
    modalSave?.addEventListener('click', () => {
        ...
        document.dispatchEvent(
            new CustomEvent('cookiePreferencesSaved', { detail: preferences }),
        );
        document.dispatchEvent(new CustomEvent('cookieConsentAccepted'));
    ...
    document.addEventListener('click', function (e) {
        if (
            e.target.classList.contains('showHideToggleCookiePreferencesModal')
        ) {
            e.preventDefault();
            showHideToggleCookiePreferencesModal();
        }
